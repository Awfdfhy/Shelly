...
// regenerated rather than kept.
// v46 (2026-08-01): adds the `x)` case to dispatch_social_post (X/Twitter
// OAuth 2.0 PKCE regular posts) plus the http_post_form/url_encode_form/
// write_pending_connector_secret_update helpers it needs. REAL BEHAVIOR
...
// v51 (2026-08-04): new 'browser-pane' agent action type (drives a live,
// on-screen Browser Pane WebView via lib/browser-pane-automation.ts's closed
// click/fill/extractText set) — new dispatch_agent_action case, new
// ACTION_BROWSER_PANE_* baked constants, and 'browser-pane' added to
// request_and_wait_approval's always-write+wait case list. Bumped so a stale
...
const ACTION_OUTPUT_INSTRUCTIONS: Record<AgentActionType, string> = {
  ...
  // browser-pane (2026-08-04): the click/fill/extractText operation itself is
  // authored on the action (selector/value), not generated by the model —
  // this only matters for a 'fill' action whose value template contains the
  // literal "{{result}}" placeholder (see dispatch_agent_action's
  // browser-pane) case), so keep the instruction generic.
  'browser-pane': 'Produce exactly the text needed for the browser action (e.g. a value to fill into a form field), if any is requested.',
};
 ...
 *
 * NOTE (2026-08-04): 'browser-pane' is deliberately NOT baked into this
 * per-index fan-out — there are no ACTION_MULTI_BROWSER_PANE_* bash arrays,
 * so a multi-action entry of this type reaches dispatch_agent_action's
 * `browser-pane)` case with every ACTION_BROWSER_PANE_* var still at its
 * single-action default ('') and fails closed with a clear "missing
 * selector"/"missing its URL allowlist" error rather than silently running
 * the wrong (or a stale) action. A single-action `agent.action` of type
 * 'browser-pane' is fully supported; only the >= 2-entry `agent.actions`
 * fan-out is out of scope for this pass — extending it means adding the
 * matching ACTION_MULTI_BROWSER_PANE_* arrays here and in generateRunScript's
 * multi-action loop, mirroring the other ACTION_MULTI_* fields exactly.
 */
...
export function generateRunScript(agent: Agent, opts: { suppressAction?: boolean; suppressErrorNotification?: boolean; autonomousCloudConsent?: boolean; autonomousCloudStop?: boolean; suppressWebCodexBake?: boolean; attended?: boolean; chainLockNonce?: string;
  ...
  // browser-pane (2026-08-04): see AgentAction.browserPaneAction's doc
  // comment (store/types.ts) and dispatch_agent_action's `browser-pane)`
  // case below for the full attended-only rationale. Baked the same way
  // intent's fields are: only populated when this IS the action type,
  // '' otherwise (defense in depth — dispatch_agent_action only reads these
  // when ACTION_TYPE is actually "browser-pane").
  const actionBrowserPane = actionType === 'browser-pane' ? agent.action?.browserPaneAction : undefined;
  const actionBrowserPaneKind = actionBrowserPane?.kind ?? '';
  const actionBrowserPaneSelector = actionBrowserPane?.selector ?? '';
  const actionBrowserPaneValue = actionBrowserPane && actionBrowserPane.kind === 'fill' ? actionBrowserPane.value : '';
  const actionBrowserPaneUrlAllowlistJson =
    actionType === 'browser-pane' ? JSON.stringify(agent.action?.browserPaneUrlAllowlist ?? []) : '[]';
...
if [ -f "$BACKEND_ERROR_FILE" ]; then
  ...
  // Final line of defense before workspaceRoot is baked into the generated
  // script's AGENT_WORKSPACE_ROOT (below) and re-anchors the driver's --cwd:
  // agent-manager.ts's createAgent/updateAgent already reject an unsafe root
  // at registration time, but this function is the single choke point every
  // path that materializes a run script passes through (including any future
  // caller that constructs an Agent without going through agent-manager.ts),
  // so re-validate here rather than trusting the field. Fable5 review 2026-08-25.
  const safeWorkspaceRoot =
    agent.workspaceRoot &&
    validateWorkspaceRoot(agent.workspaceRoot, [
      getHomePath(),
...
AGENT_WORKSPACE_ROOT=${shellQuote(safeWorkspaceRoot ?? '')}
...
ACTION_BROWSER_PANE_URL_ALLOWLIST=${shellQuote(actionBrowserPaneUrlAllowlistJson)}
...
# 120s repeatedly timed out in practice — reaching the notification shade,
# expanding it, and tapping Review/Allow ate most of the window on its own,
# before the user even read the request. Bumped to 300s; still bounded (not
# unbounded) so an unattended/abandoned request doesn't hang the run forever.
ACTION_APPROVAL_TIMEOUT_SECONDS="\${SHELLY_AGENT_ACTION_APPROVAL_TIMEOUT_SECONDS:-300}"
...
// sync manually (three copies: here, scripts/shelly-plan-executor.js's
// enforcePlanCharLimit, and the .ts source) since these run in three
// different execution contexts with no shared module system.
const FULLWIDTH_RE = /[\\u3000-\\u303F\\u3040-\\u309F\\u30A0-\\u30FF\\u3400-\\u4DBF\\u4E00-\\u9FFF\\uF900-\\uFAFF\\uFF00-\\uFFEF\\uAC00-\\uD7AF]/;
const xWeightedLength = (s) => {
  let total = 0;
  for (const ch of Array.from(s)) total += FULLWIDTH_RE.test(ch) ? 2 : 1;
  return total;
};
const chars = Array.from(text);
if (!limit || xWeightedLength(text) <= limit) {
  ...
  for (let i = 0; i < chars.length; i++) {
    const w = FULLWIDTH_RE.test(chars[i]) ? 2 : 1;
    if (acc + w > budget) { cutIdx = i; break; }
    acc += w;
  }
...
# (RFC 3986, via Node's encodeURIComponent — same "delegate to node for
# anything beyond trivial string munging" convention http_post_json's
# WordPress Basic-auth base64 step already uses). Returns empty + failure if
# node is unavailable; callers that need this (the X token endpoint) already
# fail closed on empty required fields.
url_encode_form() {
  if node_usable; then
    SHELLY_URLENC_VALUE="$1" shelly_node -e 'process.stdout.write(encodeURIComponent(process.env.SHELLY_URLENC_VALUE || ""));' 2>/dev/null || true
  fi
}
...
# queue already uses this pattern for). Needed because a detached background
# script can write files but cannot call into Expo SecureStore directly — the
# ONLY current user is the x) case's rotated OAuth refresh token, but the
# shape is generic (connectorId/field/value) in case another platform needs
# it later. Best-effort: a failure to persist here means the NEXT dispatch's
# token refresh will fail closed with a clear "reconnect it in Settings"
# error rather than silently posting with a stale/invalid token.
write_pending_connector_secret_update() {
  _wpcsu_connector_id="$1"
  _wpcsu_field="$2"
  _wpcsu_value="$3"
  _wpcsu_dir="$HOME/.shelly/pending-connector-secret-updates"
  mkdir -p "$_wpcsu_dir" 2>/dev/null || return 1
  _wpcsu_value_json=$(json_escape_text "$_wpcsu_value")
  _wpcsu_file="$_wpcsu_dir/$_wpcsu_connector_id-$_wpcsu_field-$$.json"
  printf '{"connectorId":"%s","field":"%s","value":"%s"}' "$_wpcsu_connector_id" "$_wpcsu_field" "$_wpcsu_value_json" > "$_wpcsu_file" 2>/dev/null || return 1
  chmod 600 "$_wpcsu_file" 2>/dev/null || true
  return 0
}
...
dispatch_social_post() {
  ...
  case "$ACTION_SOCIAL_PLATFORM" in
    ...
    bluesky)
      ...
      ;;
    x)
      # OAuth 2.0 PKCE. Every dispatch exchanges the stored refresh token for a
      # fresh access token — X access tokens expire in ~2h, which an
      # unattended agent running for weeks would routinely outlive, so this
      # mirrors bluesky's createSession-per-post shape (re-auth every call)
      # rather than tracking an access-token expiry window. X rotates the
      # refresh token on EVERY exchange, so a successful refresh here writes
      # the NEW token to a pending-update file that app/_layout.tsx drains
      # into SecureStore on next launch (same poll-and-drain shape the
      # deep-link queue already uses) — skip that and the very next post
      # fails with invalid_grant. Regular posts only (POST /2/tweets); the
      # Articles long-form endpoint needs a title + DraftJS content_state, not
      # plain text, and is intentionally not wired to this action yet.
      sp_refresh="$(social_connector_env REFRESHTOKEN)"
      sp_client="$(social_connector_env CLIENTID)"
      if [ -z "$sp_refresh" ] || [ -z "$sp_client" ]; then
        social_post_error "X connector is missing its refresh token or client id — reconnect it in Settings."
        return 1
      fi
      sp_x_refresh_body="grant_type=refresh_token&refresh_token=$(url_encode_form "$sp_refresh")&client_id=$(url_encode_form "$sp_client")"
      sp_x_refresh_out="$TMP_DIR/social-post-x-refresh-$AGENT_ID-$$.json"
      set +e
      http_post_form "https://$sp_host/2/oauth2/token" "$sp_x_refresh_body" "$sp_x_refresh_out" "$sp_error"
      sp_x_refresh_rc=$?
      set -e
      sp_x_access=""
      sp_x_new_refresh=""
      if [ "$sp_x_refresh_rc" -eq 0 ] && [ -s "$sp_x_refresh_out" ]; then
        sp_x_access="$(json_field_file "$sp_x_refresh_out" "access_token")"
        sp_x_new_refresh="$(json_field_file "$sp_x_refresh_out" "refresh_token")"
      fi
      rm -f "$sp_x_refresh_out"
      if [ -z "$sp_x_access" ]; then
        sp_x_err=$(redact_secrets_text "$sp_error" 2>/dev/null | head -c 240 | tr '\\n' ' ')
        rm -f "$sp_error"
        social_post_error "X token refresh failed (exit $sp_x_refresh_rc): $sp_x_err"
        return 1
      fi
...
write_action_approval_request() {
  ...
  browser_pane_value_json=$(json_escape_text "$browser_pane_value_resolved")
  browser_pane_url_allowlist_json=$(json_escape_text "$ACTION_BROWSER_PANE_URL_ALLOWLIST")
  ts_seconds=$(date +%s)
  ...
  # — harmless/unused for every other type. browser-pane is a THIRD type that
  # always reaches here (see the case-list above), but is deliberately EXCLUDED
  # from ever auto-accepting, even when the approval-mode default is 'auto' —
...
dispatch_agent_action() {
  ...
  case "$ACTION_TYPE" in
    ...
    browser-pane)
      ...
      # store/types.ts for the full rationale. The actual side effect
      # (resolving WHICH Browser Pane via resolveTargetBrowserPaneId, then
      ...
      if [ "\${AGENT_AUTONOMOUS:-0}" = "1" ] || [ "\${SHELLY_RUN_UNATTENDED:-0}" = "1" ]; then
        ACTION_DISPATCH_STATUS="skipped"
        ACTION_DISPATCH_MESSAGE="Browser actions require an attended Review with a visible Browser Pane."
        write_native_notification_request "error" "$ACTION_DISPATCH_MESSAGE" || true
        return 1
      fi
      if [ "$ACTION_BROWSER_PANE_KIND" != "click" ] && [ "$ACTION_BROWSER_PANE_KIND" != "fill" ] && [ "$ACTION_BROWSER_PANE_KIND" != "extractText" ]; then
        ACTION_DISPATCH_STATUS="error"
        ACTION_DISPATCH_MESSAGE="Browser action has an invalid kind."
        write_native_notification_request "error" "$ACTION_DISPATCH_MESSAGE" || true
        return 1
      fi
      if [ -z "$ACTION_BROWSER_PANE_SELECTOR" ]; then
        ACTION_DISPATCH_STATUS="error"
        ACTION_DISPATCH_MESSAGE="Browser action is missing a CSS selector."
        write_native_notification_request "error" "$ACTION_DISPATCH_MESSAGE" || true
        return 1
      fi
      if [ -z "$ACTION_BROWSER_PANE_URL_ALLOWLIST" ] || [ "$ACTION_BROWSER_PANE_URL_ALLOWLIST" = "[]" ]; then
        ACTION_DISPATCH_STATUS="error"
        ACTION_DISPATCH_MESSAGE="Browser action is missing its URL allowlist."
        write_native_notification_request "error" "$ACTION_DISPATCH_MESSAGE" || true
        return 1
      fi
      ...
      ACTION_DISPATCH_MESSAGE="Browser action fired: $ACTION_BROWSER_PANE_KIND $ACTION_BROWSER_PANE_SELECTOR"
      return 0
      ;;
    *)
      ACTION_DISPATCH_STATUS="error"
...
# Same shape as http_post_json but Content-Type: application/x-www-form-urlencoded
# with a raw pre-encoded body string (not a JSON file) — needed for X's OAuth
# 2.0 token endpoint, which (like every RFC 6749 token endpoint) rejects a
# JSON body. Scoped narrower than http_post_json: it does NOT route through
# the capability broker (SHELLY_CAP_BROKER=1) yet, since the broker's egress
# primitives currently assume a JSON body/AUTH_REF pair — this call carries no
# Authorization header at all (client_id/refresh_token travel in the body,
# which is the token endpoint's own spec, not a bearer secret this shell
# holds), so failing closed when the broker is requested is the safe default
# until form-encoded broker support exists.
http_post_form() {
  url="$1"
  form_body="$2"
  out_file="$3"
  err_file="$4"
  if [ "\${SHELLY_CAP_BROKER:-0}" = "1" ]; then
    echo "capability broker requested (SHELLY_CAP_BROKER=1) but http_post_form has no broker route yet; refusing to send unbrokered." > "$err_file"
    return 44
  fi
  if node_usable; then
    shelly_node - "$url" "$form_body" > "$out_file" 2> "$err_file" <<'NODEEOF'
const fs = require('fs');
const http = require('http');
const https = require('https');

const [urlText, bodyText] = process.argv.slice(2);
...
ensure_local_llm_server() {
  ...
  if [ "$use_linker64" = 1 ] && [ -x /system/bin/linker64 ]; then
    ...
    # doesn't need it. Keep in lockstep with scripts/shelly-local-llm-ensure.sh
    # (+ its asset copy).
    (
      unset LD_PRELOAD
...
# needing a re-save/re-bake. A per-agent override baked at generation time
# (ACTION_APPROVAL_MODE_OVERRIDE, from Agent.requireActionApproval) always
# wins when set.
#
# Fable5 review 2026-08-25 (supersedes the 2026-07-14 "defaults off"
# directive this comment used to cite): the old form only went manual on an
# EXPLICIT "1" and fell through to auto otherwise — so a missing/empty .env
# (a sourcing failure, a fresh install before the flag was ever synced) read
# as "no approval needed" for actions with real side effects. Flipped to the
# opposite failure mode: only an EXPLICIT "0" opts back into auto; anything
# else, including unset, requires manual approval.
if [ -n "$ACTION_APPROVAL_MODE_OVERRIDE" ]; then
  ACTION_APPROVAL_MODE="$ACTION_APPROVAL_MODE_OVERRIDE"
elif [ "\${SHELLY_DEFAULT_REQUIRE_ACTION_APPROVAL:-1}" = "0" ]; then
  ACTION_APPROVAL_MODE="auto"
else
